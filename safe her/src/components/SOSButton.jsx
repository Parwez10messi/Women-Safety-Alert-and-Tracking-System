...
export default function SOSButton({ contacts, onSOSTriggered }) {
  ...
  function handleSOSClick() {
    ...
    if (!navigator.geolocation) {
      setIsLocating(false);
      setError('Unable to fetch location. Geolocation is not supported by your browser.');
      return;
    }
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        ...
        if (err.code === 1) { // PERMISSION_DENIED
          setError('Please allow location access to trigger the SOS alert.');
        } else if (err.code === 2) { // POSITION_UNAVAILABLE
          setError('Unable to fetch location. Please ensure your device GPS is enabled.');
        } else if (err.code === 3) { // TIMEOUT
          setError('Unable to fetch location. The request timed out. Please try again.');
        } else {
          setError('Unable to fetch location. Please check your GPS/location settings.');
        }
      },
      options
    );
  ...
  return (
    <div className="glass-panel p-6 border border-safety-purple-100 flex flex-col items-center justify-center relative overflow-hidden">
      {/* Background mandala decoration inside card */}
      <div className="absolute -left-10 -bottom-10 w-24 h-24 opacity-[0.04] text-safety-purple-700 pointer-events-none animate-spin-slow">
      ...
      <div className="text-center mb-6 w-full relative z-10">
        <h2 className="text-xl font-bold text-slate-805 flex items-center justify-center space-x-1.5">
          <AlertOctagon className="h-5 w-5 text-safety-purple-700 shrink-0" />
      ...
      {error && (
        <div className="mb-4 w-full p-3.5 rounded-xl bg-red-50 border border-red-100 flex items-start space-x-2 text-red-700 text-xs animate-fadeIn relative z-10">
      ...
      {!activeAlert ? (
        <div className="flex flex-col items-center py-6 relative z-10">
          ...
          <div className="relative flex items-center justify-center">
            {isLocating && (
              <div className="absolute w-36 h-36 bg-safety-purple-500/20 rounded-full animate-ping-slow"></div>
            )}
            <div className={`absolute w-32 h-32 bg-safety-purple-500/10 rounded-full ${isLocating ? 'animate-pulse' : 'sos-button-glow'}`}></div>
            
            <button
              onClick={handleSOSClick}
              disabled={isLocating}
              className={`relative w-28 h-28 bg-gradient-to-tr from-safety-purple-500 via-safety-purple-700 to-safety-pink-500 hover:scale-[1.03] active:scale-95 text-white font-black text-3xl rounded-full shadow-2xl transition duration-200 border-4 border-white flex flex-col items-center justify-center tracking-wider disabled:opacity-80 disabled:cursor-wait select-none`}
            >
              {isLocating ? (
