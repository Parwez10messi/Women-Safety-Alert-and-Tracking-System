...
export default function Dashboard() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
      ...
      {/* Welcome Banner */}
      <div className="relative overflow-hidden bg-safety-purple-800 rounded-3xl p-6 sm:p-8 mb-8 text-white shadow-xl flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        {/* Background Mandala overlay decoration */}
        <div className="absolute -right-10 -bottom-10 w-40 h-40 opacity-[0.08] text-white pointer-events-none animate-spin-slow">
        ...
        <div className="relative z-10 flex items-center space-x-4">
          <div className="p-3 bg-white/10 rounded-2xl hidden sm:block border border-white/10">
            <Shield className="h-8 w-8 text-safety-pink-100" />
          </div>
          <div>
            <div className="flex items-center space-x-2 text-safety-pink-200 text-xs font-bold tracking-wider uppercase">
              <Sparkles className="h-3.5 w-3.5 text-safety-pink-200" />
              <span>SafeHer Portal</span>
            </div>
            <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight mt-1">
              Welcome back, {userProfile?.name || currentUser?.displayName || 'User'}!
            </h1>
            <p className="text-sm text-safety-pink-50 mt-1">
